...
use crate::{
    CurrentCtx, Stream,
    bindings::{
        HCgraph, HCgraphExec, HCgraphNode,
        hcStreamCaptureMode::hcStreamCaptureModeThreadLocal as LOCAL,
    },
};
...
//             return;
//         }
//         // 不需要在上下文中创建和释放 hc graph
//         let mut graph = null_mut();
//         driver!(hcGraphCreate(&mut graph, 0));
//         assert!(!graph.is_null());
//         // 但 hc graph 实例化需要在上下文中
//         let mut exec = null_mut();
//         driver!(
//             hcErrorInvalidContext,
//             hcGraphInstantiateWithFlags(
//                 &mut exec,
//                 graph,
//                 hcGraphInstantiateFlags::hcGraphInstantiateFlagAutoFreeOnLaunch as _
//             )
...
//             // hc graph 会将 kernel 用到的参数拷贝保存在节点中
...
//             // driver!(
//             //     hcErrorStreamCaptureUnsupported,
//             //     hcGraphLaunch(exec.as_raw(), stream.as_raw())
//             // );
//             // std::mem::forget(stream);

//             driver!(
//                 hcErrorStreamCaptureUnsupported,
//                 hcGraphExecDestroy(exec.as_raw())
//             );
//             std::mem::forget(exec);

//             driver!(
//                 hcErrorStreamCaptureUnsupported,
//                 hcModuleUnload(module.as_raw())
//             );
