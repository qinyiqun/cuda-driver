...
impl Device {
    ...
    pub fn context(&self) -> Context {
        ...
        driver!(hcCtxCreate(&mut ctx, 0, dev));
        driver!(hcCtxPopCurrent(null_mut()));
        Context {
            ctx,
            dev,
            primary: false,
    ...
    pub fn retain_primary(&self) -> Context {
        let dev = unsafe { self.as_raw() };
        let mut ctx = null_mut();
        driver!(hcDevicePrimaryCtxRetain(&mut ctx, dev));
        Context {
            ctx,
            dev,
            primary: true,
...
impl Drop for Context {
    ...
    fn drop(&mut self) {
        if self.primary {
            // driver!(hcDevicePrimaryCtxReset(self.dev))
        } else {
            driver!(hcCtxDestroy(self.ctx))
        }
    }
}
...
impl Context {
    ...
    pub fn apply<T>(&self, f: impl FnOnce(&CurrentCtx) -> T) -> T {
        driver!(hcCtxPushCurrent(self.ctx));
        let ans = f(&CurrentCtx(self.ctx));
        let mut top = null_mut();
        driver!(hcCtxPopCurrent(&mut top));
...
#[repr(transparent)]
pub struct CurrentCtx(HCcontext);
...
impl CurrentCtx {
    ...
    pub fn mem_info(&self) -> (MemSize, MemSize) {
        let mut free = 0;
        let mut total = 0;
        driver!(hcMemGetInfo(&mut free, &mut total));
        (free.into(), total.into())
    }

    /// 如果存在当前上下文，在当前上下文上执行依赖上下文的操作。
    #[inline]
    pub fn apply_hcrrent<T>(f: impl FnOnce(&Self) -> T) -> Result<T, NoCtxError> {
        let mut raw = null_mut();
        driver!(hcCtxGetCurrent(&mut raw));
        if !raw.is_null() {
            Ok(f(&Self(raw)))
        } else {
            Err(NoCtxError)
    ...
    /// # Safety
    ///
    /// The `raw` context must be the hcrrent pushed context.
    #[inline]
    pub unsafe fn apply_hcrrent_unchecked<T>(raw: HCcontext, f: impl FnOnce(&Self) -> T) -> T {
        f(&Self(raw))
    }

    /// Designates `raw` as the hcrrent context.
    ///
    /// # Safety
    ///
    /// The `raw` context must be the hcrrent pushed context.
    ...
    #[inline]
    pub unsafe fn from_raw<'ctx>(raw: &HCcontext) -> &'ctx Self {
    ...
    #[inline]
    pub unsafe fn wrap_raw<T: Unpin + 'static>(&self, rss: T) -> RawContainer<HCcontext, T> {
...
impl CurrentCtx {
    ...
    pub fn unlock_page<T>(&self, slice: &[T]) {
        driver!(hcHostUnregister(slice.as_ptr() as _));
    }
}
...
fn test_primary() {
    ...
    let mut flags = 0;
    let mut active = 0;
    driver!(hcDevicePrimaryCtxGetState(
        dev.as_raw(),
        &mut flags,
        &mut active
    ));
    ...
    driver!(hcDevicePrimaryCtxRetain(&mut pctx, dev.as_raw()));
    assert!(!pctx.is_null());
    ...
    driver!(hcCtxGetCurrent(&mut pctx));
    assert!(pctx.is_null());
}
